...
Most users configure this recipe from `compose/ubuntu-libfaketime/.env`.
Start with these values, then expand the groups below when you need deeper control:

```dotenv
UBUNTU_BASE_IMAGE=ubuntu:24.04
UBUNTU_IMAGE_NAME=docker-cookbook/ubuntu-libfaketime:24.04
UBUNTU_CONTAINER_NAME=ubuntu-libfaketime
TZ=UTC
FAKETIME=
```
...
- `UBUNTU_BASE_IMAGE` (default: `ubuntu:24.04`): Base Ubuntu image used by the Dockerfile.
- `UBUNTU_IMAGE_NAME` (default: `docker-cookbook/ubuntu-libfaketime:24.04`): Compose image name and tag.
...
- `UBUNTU_CONTAINER_NAME` (default: `ubuntu-libfaketime`): Compose container name.
